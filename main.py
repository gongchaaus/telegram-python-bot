...
async def sales(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if store_id:
        ...
        if((not gross_sales is None) and (gross_sales>0)):
            ...
            reply_text = f'{store_name}s Adjusted Gross Sales for Bonus Calculation on {today_str}: ${gross_bonus_sales} incl. GST'
            await update.message.reply_text(reply_text)
            message += reply_text
            ...
            await update.message.reply_text(f'store_name: {store_name}')
            await update.message.reply_text(f'today_str: {today_str}')
